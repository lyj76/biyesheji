...
%TapLens是抽头的长度；HiddenSize隐藏层大小；LearningRate学习率； MaxEpochs最大轮次下降训练；
%
...
%   Output: Concise debug info (Sync MSE, Train/Val BER).
    ...
    %% 2) Synchronization (Linear Probe探测找到时延和对应的off作为正确输入
    %得到d（第一个tx_n对应的第一个rx_n索引2*d）和off（奇数还是偶数）
    %粗略定位，基于最小mse；没有截断数据，仅仅为了定位
