...
% RNN_WD_Implementation (Direct Prediction - No Residual)
%   Structure: Windowed Decision Feedback RNN.
...
%   Output: Estimated Symbol Level y(n) (Normalized)
    ...
    %% 2) Grid Search for Best Delay (Using Linear Probe)
    ...
    disp('---------------------------------------------------------');
    disp(['[RNN_WD Debug] Training MSE: ', num2str(mse_train, '%.5e')]);
    disp(['[RNN_WD Debug] Training BER: ', num2str(ber_train, '%.5e')]);
    disp('---------------------------------------------------------');
    % --- DEBUG INFO END ---
    ...
    % Determine Levels for Hard Decision (from Training Data distribution)
